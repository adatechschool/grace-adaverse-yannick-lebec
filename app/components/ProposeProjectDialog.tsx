...
type FormProps = Props & { onClose: () => void };

function DialogForm({ promotions, adaProjects, onClose }: FormProps) {
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        ...
        <form action={action} className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" name="title" required className="w-full border rounded px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">URL GitHub *</label>
            <input type="url" name="githubUrl" required className="w-full border rounded px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">URL Démo *</label>
            <input type="url" name="demoUrl" required className="w-full border rounded px-3 py-2 text-sm" />
          </div>
